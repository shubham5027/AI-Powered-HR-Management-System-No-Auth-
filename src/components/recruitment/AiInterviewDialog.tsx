...
export function AiInterviewDialog({ 
  ...
  const handleRecordingComplete = (audio: Blob, image: Blob | null) => {
    setAudioBlob(audio);
    setImageBlob(image);
    setStep('processing');
    processInterviewRecording(audio, image);
  };
  
  const processInterviewRecording = async (audio: Blob, image: Blob | null) => {
    setIsLoading(true);
    try {
      // Default candidate ID if no candidate is selected
      const candidateId = candidate?.id || 'demo-candidate';
      
      // Process the interview recording with both audio and image
      const result = await processInterview(audio, image, candidateId);
      ...
      <div className="max-w-md mx-auto pt-4">
        <div className="space-y-4">
          ...
          <div className="flex justify-between text-sm">
            ...
            <span className={imageBlob ? "text-blue-500 animate-pulse" : "text-gray-400"}>
              {imageBlob ? "In progress..." : "Skipped (no image)"}
            </span>
          </div>
          <div className="flex justify-between text-sm">
            <span>Analyzing sentiment</span>
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="bg-blue-50 border border-blue-100 rounded-md p-4">
              ...
              <p className="text-sm text-blue-700 mb-4">
                This enhanced interview feature uses AI to analyze both audio and visual cues:
              </p>
              <ol className="text-sm space-y-2 text-blue-700 list-decimal pl-4">
                <li>Recording the interview (audio and optional video)</li>
                <li>Transcribing the recording using speech-to-text</li>
                <li>Analyzing face for emotions and attentiveness (if enabled)</li>
                <li>Analyzing the transcript for sentiment and key topics</li>
                <li>Generating a comprehensive AI recommendation</li>
              </ol>
            </div>
    ...
    <Dialog open={open} onOpenChange={(isOpen) => {
      ...
      <DialogContent className="sm:max-w-[700px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Sparkles className="h-5 w-5 text-purple-500" />
            Enhanced AI-Driven Interview
          </DialogTitle>
          <DialogDescription>
            Conduct an AI-powered interview with automated analysis of candidate responses and visual cues.
          </DialogDescription>
        </DialogHeader>
